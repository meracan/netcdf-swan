...
"""
    ...
    reads from current directory, but may need to search if path is not specified
...
class NodeMap:
  ...
  def swan_xyzread(self, xyzfile):
    ...
    self.has_timesteps = True
    return xyz


  def load_mat(self, filename):
    """ if folder is given, searches the entire folder.
        ...
        need  'np.squeeze()'  to be able to convert into a pandas dataframe
  ...
  def sort_coords(self):
    ...
    for i in tqdm(range(len(lon_i_sort))):
      #self.lon_sort.append( (self.lon[lon_i_sort[i]], lon_i_sort[i]) )
      self.lon_sort.append( lon_i_sort[i] )
    
    print("sorting latitudes...")
    for i in tqdm(range(len(lat_i_sort))):
      #self.lat_sort.append( (self.lat[lat_i_sort[i]], lat_i_sort[i]) )
